...
> 變更日期與歷程一律看 git 記錄；**請勿**在本檔手寫「最後更新」。
...
3. **分享頁**（簡單）：預覽圖卡 → 分享到 IG 限動（含簡短教學，文案／視覺須 6w7 原創，禁止抄 NGL）。教學以彈出視窗播放手機規格影片 `https://cdn.6w7.link/walkthrough/guide_share_to_ig_001.mp4`（分享頁「怎麼發到 IG 限動？／限動教學」與分享圖卡內入口）。首次進入分享頁用與限動教學相同的懸浮指標帶路：① 確認語言 ② 全畫面 logo 置中片刻 ③ 換頭貼 ④ 改提示 ⑤ 看限動教學或直接帶去做分享。若看教學：引導先在旁邊等影片播完，再帶去按彈窗裡的「分享到 IG 限動」，接著與分享路徑相同：複製專屬短網址 → 提示「分享此圖 → Instagram → 限時動態」→ 按分享此圖。走完後一般帳號寫入本機之後不再出現。示範帳號每次登入都會再跳出。登入後頂欄帳號鈕左側有羅盤鈕可再開一次導覽。
...
- 回覆圖卡（產圖下載，非抄市面上匿名問答站版型）—— Inbox「限動圖卡」：1080×1920 PNG，6w7 原創版型（側欄色條＋墨色底＋6w7 品牌腳）
...
POST   /api/v1/media                       # 上傳圖片（需登入；示範帳號寫入自己的 sandbox，圖檔 30 分鐘後刪）
...
GET    /api/v1/cron/demo-media             # 清過期示範裝扮圖（Vercel Cron；`CRON_SECRET`）
```
...
## 11.3 RPG 封測（使用者已要求逐步開發）

- 允許新增獨立遊戲模組：Next.js + Phaser 3 客戶端、同 repo 的 TypeScript Colyseus 常駐服務（Node.js 22 LTS），npm 單一鎖檔。Vercel 保留網站，Oracle 承接即時遊戲，不改寫現有匿名問答。
- 第一階段入口 `/tools/play`（避免占用既有 `/{username}`），不加入正式導覽、搜尋索引或廣告。封測先提供繁中，正式公開前補齊四語。
- 第一階段為臨時邀請世界，每房最多 4 人：多元人類體型、外觀選擇、小屋／街區／林地、表情、合作打小魔物。外觀僅存本機，世界與獎勵是暫存；不得宣稱已綁定 IG、永久個人小屋或正式存檔。
- 紙娃娃提供纖瘦、勻稱、精實、健壯、胖壯、圓潤預設，可調肩部、二頭肌、三頭肌、前臂、胸腹與腿部；髮型、臉型、鬍型、六區體毛密度各自設定。衣褲鞋帽、項鍊、身上配件與手持物分層，最低衣著固定為可換款／色的三角或四角內褲。以細緻像素角色共用於預覽與遊戲，文案歡迎各種外型玩家。外觀採版本化白名單並遷移本機舊設定；調整外觀不得重置單人遊戲進度。
- 房主出門時家中無分身；離線且仍有訪客時顯示睡眠角色。會合由房主控制；斷線有重連緩衝。邀請世界在全員離開或服務重啟後失效。
- 邀請票證由 HTTP API 簽發，驗證來源、限流、輸入大小與外觀白名單。正式環境須明確啟用、設定獨立簽章密鑰與封測通關碼；不得使用開發預設值。遊戲端驗票、限制房間數／訊息率／封包大小，伺服器裁定移動、碰撞與戰鬥。
- Oracle 與 GoodJianPlus 共用，目前僅 1 GB RAM。先完成本機與多人測試、準備獨立 systemd 與反向代理設定再部署；不得停止／取代 Django、其資料庫或原有代理設定。
- 下一階段才新增 User 關聯的角色／小屋模型與 migration、永久專屬連結、帳號授權、封鎖檢舉與正式存檔，完成後再對外開放工具。
