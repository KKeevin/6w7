...
3. **分享頁**（簡單）：預覽圖卡 → 第一步複製連結 → 第二步分享到 IG 限動（含簡短教學，文案／視覺須 6w7 原創，禁止抄 NGL）。教學以彈出視窗播放手機規格影片 `https://cdn.6w7.link/walkthrough/guide_share_to_ig_001.mp4`（分享頁「怎麼發到 IG 限動？／限動教學」與分享圖卡內入口）。首次進入分享頁以懸浮指標框住該按鈕；「不要再顯示」後一般帳號寫入本機之後不再出現。示範帳號每次登入都會再跳出（此次連線可先關掉）。
